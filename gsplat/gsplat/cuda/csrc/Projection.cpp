...
projection_ewa_3dgs_fused_fwd(
    ...
    launch_projection_ewa_3dgs_fused_fwd_kernel(
        ...
        calc_compensations ? at::optional<at::Tensor>(compensations)
                           : c10::nullopt,
        calc_opacities ? at::optional<at::Tensor>(opacities_out)
                       : c10::nullopt             
    );
    return std::make_tuple(radii, means2d, depths, conics, compensations, opacities_out);
}
...
projection_ewa_3dgs_fused_bwd(
    ...
    const at::optional<at::Tensor> opacities_out,
    // grad outputs
    const at::Tensor v_means2d,                     // [..., C, N, 2]
    ...
    if (opacities_in.has_value()) {
        CHECK_INPUT(opacities_in.value());
    }
    if (v_opacities_out.has_value()) {
        CHECK_INPUT(v_opacities_out.value());
        assert(opacities_in.has_value());
    }
    ...
    at::Tensor v_opacities_in;
    if (opacities_in.has_value()) {
        v_opacities_in = at::zeros_like(opacities_in.value());
    }
    ...
    return std::make_tuple(v_means, v_covars, v_quats, v_scales, v_viewmats, v_opacities_in);
}
