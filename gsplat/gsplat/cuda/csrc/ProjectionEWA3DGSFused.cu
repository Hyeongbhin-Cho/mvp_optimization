...
__global__ void projection_ewa_3dgs_fused_fwd_kernel(
    ...
    const scalar_t *__restrict__ opacities_in, // [B, N] or [B, N, K, 1] optional [2026-01-16 / hyeongbhin]
    ...
    const int32_t sh_degree_opacity,    // SH degree optional [2026-01-16 / hyeongbhin]
    // outputs
    ...
    scalar_t *__restrict__ opacities_out     // [B, C, N] optional
) {
    ...
    // [2026-01-16 / hyeongbhin] opacity output
    if (opacities_out != nullptr) {
        opacities_out[idx] = opacity;
    }
}
...
void launch_projection_ewa_3dgs_fused_fwd_kernel(
    ...
    const at::optional<at::Tensor> opacities_in, // [..., N] or [..., N, K, 1] optional [2026-01-16 / hyeongbhin]
    ...
    const float radius_clip,
    const CameraModelType camera_model,
    const int32_t sh_degree_opacity,        // optional [2026-01-16 / hyeongbhin]
    // outputs
    ...
    at::optional<at::Tensor> opacities_out // [..., C, N] optional [2026-01-16 / hyeongbhin]
) {
                ...
                <<<grid,
                   ...
                   at::cuda::getCurrentCUDAStream()>>>(
                    ...
                    scales.has_value() ? scales.value().data_ptr<scalar_t>()
                                       : nullptr,
                    opacities_in.has_value() ? opacities_in.value().data_ptr<scalar_t>() // [2026-01-16 / hyeongbhin]
                                         : nullptr,
                    ...
                    compensations.has_value()
                        ...
                        : nullptr,
                    opacities_out.has_value() // [2026-01-16 / hyeongbhin]
                        ? opacities_out.value().data_ptr<scalar_t>()
                        : nullptr
                );
        }
    );
...
__global__ void projection_ewa_3dgs_fused_bwd_kernel(
    ...
    const scalar_t *__restrict__ quats,    // [B, N, 4] optional
    const scalar_t *__restrict__ scales,   // [B, N, 3] optional
    const scalar_t *__restrict__ opacities_in, // [B, N] or [B, N, K, 1] optional [2026-01-18 / hyeongbhin]
    const scalar_t *__restrict__ viewmats, // [B, C, 4, 4]
    const scalar_t *__restrict__ Ks,       // [B, C, 3, 3]
    const uint32_t image_width,
    const uint32_t image_height,
    const float eps2d,
    const CameraModelType camera_model,
    const int32_t sh_degree_opacity,
    // fwd outputs
    ...
    const scalar_t *__restrict__ opacities_out, // [B, C, N] optional [2026-01-18 / hyeongbhin]
    // grad outputs
    ...
    const scalar_t *__restrict__ v_opacities_out, // [B, C, N] optional [2026-01-18 / hyeongbhin]
    // grad inputs
    ...
    scalar_t *__restrict__ v_opacities_in// [B, N, K, 1] optional
) {
    // parallelize over B * C * N.
    uint32_t idx = cg::this_grid().thread_rank();
...
void launch_projection_ewa_3dgs_fused_bwd_kernel(
    ...
    const at::optional<at::Tensor> quats,  // [..., N, 4] optional
    const at::optional<at::Tensor> scales, // [..., N, 3] optional
    const at::optional<at::Tensor> opacities_in, // [..., N, K, 1] optional
    const at::Tensor viewmats,             // [..., C, 4, 4]
    const at::Tensor Ks,                   // [..., C, 3, 3]
    const uint32_t image_width,
    const uint32_t image_height,
    ...
    const at::optional<at::Tensor> opacities_out,  // [..., C, N] optional
    // grad outputs
    const at::Tensor v_means2d,                     // [..., C, N, 2]
    const at::Tensor v_depths,                      // [..., C, N]
    const at::Tensor v_conics,                      // [..., C, N, 3]
    const at::optional<at::Tensor> v_compensations, // [..., C, N] optional
    const at::optional<at::Tensor> v_opacities_out,     // [..., C, N] optional
    ...
    at::Tensor v_opacities_in // [... N, K, 1]
) {
    uint32_t N = means.size(-2);    // number of gaussians
    uint32_t C = viewmats.size(-3); // number of cameras
    uint32_t B = means.numel() / (N * 3); // number of batches
            ...
            projection_ewa_3dgs_fused_bwd_kernel<scalar_t>
                <<<grid,
                   ...
                   at::cuda::getCurrentCUDAStream()>>>(
                    ...
                    opacities_in.has_value() ? opacities_in.value().data_ptr<scalar_t>()
                                             : nullptr,
                    viewmats.data_ptr<scalar_t>(),
                    Ks.data_ptr<scalar_t>(),
                    image_width,
                    ...
                    v_compensations.has_value()
                        ...
                        : nullptr,
                    v_opacities_out.has_value()
                        ? v_opacities_out.value().data_ptr<scalar_t>()
                        : nullptr,
                    ...
                    viewmats_requires_grad ? v_viewmats.data_ptr<scalar_t>()
                                           : nullptr,
                    v_opacities_in.defined() ? v_opacities_in.data_ptr<scalar_t>()
                                               : nullptr
                );
        }
    );
