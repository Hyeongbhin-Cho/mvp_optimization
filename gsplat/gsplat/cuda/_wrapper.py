...
def fully_fused_projection(
    ...
    if opacities is not None:
        ...
        if sh_degree_opacity < 0:
            ...
            K = (sh_degree_opacity + 1) ** 2
            assert opacities.shape[-2] >= K, opacities.shape
...
class _FullyFusedProjection(torch.autograd.Function):
    ...
    def forward(
        ...
        camera_model: Literal["pinhole", "ortho", "fisheye", "ftheta"] = "pinhole",
        opacities_in: Optional[Tensor] = None,  # [..., N] or [..., N, K, 1] or None
        calc_opacities: bool = False,
        ...
        radii, means2d, depths, conics, compensations, opacities_out = _make_lazy_cuda_func(
        ...
        if not calc_compensations:
            compensations = None
        if not calc_opacities:
            opacities_out = None
        ctx.save_for_backward(
            means, covars, quats, scales, viewmats, Ks, radii, conics, compensations, opacities_in, opacities_out
        )
        ctx.width = width
        ctx.height = height
        ctx.eps2d = eps2d
        ...
        return radii, means2d, depths, conics, compensations, opacities_out

    @staticmethod
    def backward(ctx, v_radii, v_means2d, v_depths, v_conics, v_compensations, v_opacities_out):
        (
            means,
            covars,
            quats,
        ...
        if v_opacities_out is not None:
            v_opacities_out = v_opacities_out.contiguous()
        v_means, v_covars, v_quats, v_scales, v_viewmats, v_opacities_in = _make_lazy_cuda_func(
        ...
        if not ctx.needs_input_grad[4]:
            v_viewmats = None
        if not ctx.needs_input_grad[14]:
            v_opacities_in = None
        return (
            v_means,
            v_covars,
