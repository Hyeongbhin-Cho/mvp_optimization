...
def rasterization(
    ...
    quats: Tensor,  # [..., N, 4]
    scales: Tensor,  # [..., N, 3]
    opacities: Tensor,  # [..., N] # [2026-01-13 / Hyeongbhin] [..., (C,) N, k, 1] for applay SH to opacity
    ...
    sh_degree_opacity: Optional[int] = None, # [2026-01-13 / Hyeongbhin] for applay SH to opacity
    packed: bool = True,
    tile_size: int = 16,
    ...
    # assert opacities.shape == batch_dims + (N,), opacities.shape # [2026-01-13 / Hyeongbhin]
    ...
    # [2026-01-13 / Hyeongbhin] for apply SH to opactiy
    if sh_degree_opacity is None:
        # treat opacity as post-activation values, should be in shape [..., N]
        assert (
            opacities.dim() == num_batch_dims + 1
            and opacities.shape == batch_dims + (N,)
        ), opacities.shape
        if distributed:
            assert (
                opacities.dim() == num_batch_dims + 1
            ), "Distributed mode only supports per-Gaussian opacities."
    else:
        # treat opacities as SH coefficients, should be in shape [..., N, K, 1] or [..., C, N, K, 1]
        # Allowing for activating partial SH bands
        assert (
            opacities.dim() == num_batch_dims + 3
            and opacities.shape[:-2] == batch_dims + (N,)
            and opacities.shape[-1] == 1    
        ) or (
            opacities.dim() == num_batch_dims + 4
            and opacities.shape[:-2] == batch_dims + (C, N)
            and opacities.shape[-1] == 1
        ), opacities.shape
        assert (sh_degree_opacity + 1) ** 2 <= opacities.shape[-2], opacities.shape
        if distributed:
            assert (
                opacities.dim() == num_batch_dims + 3
            ), "Distributed mode only supports per-Gaussian opacities."
    ...
    if with_ut:
        ...
        proj_results = fully_fused_projection(
            ...
            calc_opacities=(sh_degree_opacity is not None),
    ...
    if packed:
        ...
        radii, means2d, depths, conics, compensations, opacities_out = proj_results
        if sh_degree_opacity is None:
